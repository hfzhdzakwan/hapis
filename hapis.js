...
module.exports = Hapis = async (Hapis, mek) => {
	try {
        ...
        const botNumber = Hapis.user.jid
        const ownerNumber = setting.NomorOwner2
		const ownerName = setting.NamaOwner
		const botName = setting.NamaBot
		const isGroup = from.endsWith('@g.us')
		let sender = isGroup ? mek.participant : mek.key.remoteJid
		let senderr = mek.key.fromMe ? Hapis.user.jid : mek.key.remoteJid.endsWith('@g.us') ? mek.participant : mek.key.remoteJid
		const totalchat = await Hapis.chats.all()
		const groupMetadata = isGroup ? await Hapis.groupMetadata(from) : ''
        ...
        const conts = mek.key.fromMe ? Hapis.user.jid : Hapis.contacts[sender] || { notify: jid.replace(/@.+/, '') }
        const pushname = mek.key.fromMe ? Hapis.user.name : conts.notify || conts.vname || conts.name || '-'
        ...
        const listmsg = (from, title, desc, list) => { // ngeread nya pake rowsId, jadi command nya ga keliatan
            let po = Hapis.prepareMessageFromContent(from, {"listMessage": {"title": title,"description": desc,"buttonText": "Pilih Disini","footerText": "© BY HAPIS OFFICIAL","listType": "SINGLE_SELECT","sections": list}}, {})
            return Hapis.relayWAMessage(po, {waitForAck: true})
        }
        ...
        const reply = (teks) => {
	      Hapis.sendMessage(from, teks, text, {quoted:mek, thumbnail: fakeimage})
        }
        const freply = (teks) => {
Hapis.sendMessage(from, teks, text,{contextInfo :{text: 'hi',
...
"externalAdReply": {
        ...
        const sendMess = (hehe, teks) => {
           Hapis.sendMessage(hehe, teks, text)
        }
        const mentions = (teks, memberr, id) => {
           (id == null || id == undefined || id == false) ? Hapis.sendMessage(from, {text: teks.trim(), jpegThumbnail: fs.readFileSync('./media/foto/thumb.jpg')}, extendedText, { sendEphemeral: true, contextInfo: { "mentionedJid": memberr } }) : Hapis.sendMessage(from, {text: teks.trim(), jpegThumbnail: fs.readFileSync('./media/foto/thumb.jpg')}, extendedText, { sendEphemeral: true, quoted: mek, contextInfo: { "mentionedJid": memberr } })
        }
        const sendText = (from, text) => {
           Hapis.sendMessage(from, text, MessageType.text)
        }
        const textImg = (teks) => {
           return Hapis.sendMessage(from, teks, text, {quoted: mek, thumbnail: fs.readFileSync('./media/foto/thumb.jpg')})
        }
       ...
       const kick = function(from, orangnya){
	       ...
	       Hapis.groupRemove(from, [i])
        }
        }

       const kickMember = async(id, target = []) => {
           let group = await Hapis.groupMetadata(id)
           let owner = group.owner.replace("c.us", "s.whatsapp.net")
           let me = Hapis.user.jid
           for (i of target) {
           if (!i.includes(me) && !i.includes(owner)) {
           await Hapis.groupRemove(to, [i])
        } else {
           await Hapis.sendMessage(id, "Not Premited!", "conversation")
           break
        }
    }
}
       const add = function(from, orangnya){
	       Hapis.groupAdd(from, orangnya)
}
       const sendKontak = (from, nomor, nama, org = "") => {
	       const vcard = 'BEGIN:VCARD\n' + 'VERSION:3.0\n' + 'FN:' + nama + '\n' + 'ORG:' + org + '\n' + 'TEL;type=CELL;type=VOICE;waid=' + nomor + ':+' + nomor + '\n' + 'END:VCARD'
	       Hapis.sendMessage(from, {displayname: nama, vcard: vcard}, MessageType.contact, {quoted: mek})
}
      const hideTag = async function(from, text){
	       let anu = await Hapis.groupMetadata(from)
	       let members = anu.participants
	       let ane = []
	       for (let i of members){
	       ane.push(i.jid)
}
	       Hapis.sendMessage(from, {text:text, jpegThumbnail:fs.readFileSync('media/foto/thumb.jpg')}, 'extendedTextMessage', {contextInfo: {"mentionedJid": ane}})
}  
      const sendWebp = async(to, url) => {
           ...
           Hapis.sendMessage(from, fs.readFileSync(asw), sticker, {sendEphemeral:true, quoted:mek})
           fs.unlinkSync(asw)
});
});
});
       ...
       const sendMediaURL = async(to, url, text="", mids=[]) =>{
           ...
           Hapis.sendMessage(to, media, type, {quoted: mek, mimetype: mime, caption: text, thumbnail: Buffer.alloc(0), contextInfo: {"mentionedJid": mids}})
      ...
      const sendFileFromUrl = async(link, type, options) => {
	       ...
	       Hapis.sendMessage(from, hasil, type, options).catch(e => {
	       fetch(link).then((hasil) => {
	       Hapis.sendMessage(from, hasil, type, options).catch(e => {
	       Hapis.sendMessage(from, { url : link }, type, options).catch(e => {
      ...
      const promoteAdmin = async function(to, target=[]){
           ...
           let g = await Hapis.groupMetadata(to)
           let owner = g.owner.replace("c.us","s.whatsapp.net")
           let me = Hapis.user.jid
           for (i of target){
           if (!i.includes(me) && !i.includes(owner)){
           const res = await Hapis.groupMakeAdmin(to, [i])
      ...
      const demoteAdmin = async function(to, target=[]){
           ...
           for (i of target){
           if (!i.includes(me) && !i.includes(owner)){
           const res = await Hapis.groupDemoteAdmin(to, [i])
           freply(`Hm..... @${mentioned[0].split('@')[0]} JABATANMU TURUNKAN SAYA IKUT SEDIH`)
				
}
}
}
          let authorname = Hapis.contacts[from] != undefined ? Hapis.contacts[from].vname || Hapis.contacts[from].notify : undefined	
          if (authorname != undefined) { } else { authorname = groupName }	
          function addMetadata(packname, author) {	
          if (!packname) packname = 'Dibuat Oleh'; if (!author) author = '© Hapis Official';author = author.replace(/[^a-zA-Z0-9]/g, '');	
     ...
     const time2 = moment().tz('Asia/Jakarta').format('HH:mm:ss')
       ...
       console.error(err)
        ...
        const troli =  {key: { fromMe: false,remoteJid: "status@broadcast", participant: '0@s.whatsapp.net'}, message: {orderMessage: {itemCount: 500, status: 200, thumbnail: fakeimage, surface: 200, message: faketroli, orderTitle: 'Hapis', sellerJid: '0@s.whatsapp.net'} } }
        ...
        const ftoko = {key: {fromMe: false,participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "16505434800@s.whatsapp.net" } : {})},message: {"productMessage": {"product": {"productImage":{"mimetype": "image/jpeg","jpegThumbnail": fs.readFileSync(`./media/foto/thumb.jpg`)},"title": `© BY HAPIS OFFICIAL`,"description": "Hapis Official", "currencyCode": "IDR","priceAmount1000": "999999","retailerId": "Hapis Official","productImageCount": 1},"businessOwnerJid": `0@s.whatsapp.net`}}}

      // Anti link
        if (isGroup && isAntiLink && !isOwner && !isGroupAdmins && isBotGroupAdmins){
            if (budy.match(/(https:\/\/chat.whatsapp.com)/gi)) {
                freply(`*「 GROUP LINK DETECTOR 」*\nSepertinya kamu mengirimkan link grup, maaf kamu akan di kick`)
                Hapis.groupRemove(from, [sender])
            }
        }
...
function banChat() {
    if (banChats == true) {
      ...
      // GAME 
             game.cekWaktuFam(Hapis, family100)
        ...
        if (game.isfam(from, family100)) {
            ...
            if (anjuy.length < 1){
                Hapis.sendMessage(from, `Semua jawaban sudah tertebak\nKirim *${prefix}family100* untuk bermain lagi`, text)
...
kma = gam1
hapis = await Hapis.prepareMessage(from, kma, image)
const buttonMessages = {
imageMessage: hapis.message.imageMessage,
contentText: text1,
footerText: desc1,
buttons: but,
headerType: 4
}
Hapis.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
}
const sendButLocation = async(id, text1, desc1, gam1, but = [], options = {}) => {
               let kma = gam1
               Hapis.sendMessage(id, {"contentText": text1,
               "footerText": desc1, 
               "buttons": but,
               "headerType": "LOCATION",
                       "locationMessage": {
      ...
      Hapis.sendMessage(id, buttonMessage, MessageType.buttonsMessage, {quoted: mek})
    };
        // Auto Read
        Hapis.chatRead(from, "read")
        //auto vn 
        await Hapis.updatePresence(from, Presence.recording)
...
danzrun = process.uptime() 
           Hapis.setStatus(`${namabot} Aktif Selama ${(danzrun)} © Creator By Hapis Official`).catch((_)=>_); //DI UBAH YATIMM
...
const fakeText = (teks) => {
Hapis.sendMessage(from, teks, text, {quoted: troli})
}          
            ...
            switch(command){
				...
				try {
				ppimg = await Hapis.getProfilePicture(`${sender.split('@')[0]}@c.us`)
				} catch {
...
`
                ...
                Hapis.sendMessage(from, buff, MessageType.image, {quoted: mek, caption: hapisganteng, contextInfo: {'mentionedJid': [sender]}})
                break
//DEWASA
    case 'saya18':
	case 'my18':
...
`
                ...
                Hapis.sendMessage(from, buffmek, MessageType.image, {quoted: mek, caption: hapisgans, contextInfo: {'mentionedJid': [sender]}})
                break
//BOCIL
case 'my11':
case 'my12':
...
break
       ...
       if (isBanned) return freply(mess.banned)
               ...
               prep = await Hapis.prepareMessageFromContent(from,{buttonsMessage},{})
...
`
               ...
               imageMsg = (await Hapis.prepareMessageMedia(fs.readFileSync(`./media/foto/menu.jpg`), 'imageMessage', {thumbnail: fs.readFileSync(`./media/foto/menu.jpg`)})).imageMessage

               buttonsMessage = {
               contentText: `${menu}`,
               footerText: 'ᴊɪᴋᴀ ᴛᴏᴍʙᴏʟ ᴅɪ ʙᴀᴡᴀʜ ᴇʀʀᴏʀ sɪʟᴀʜᴋᴀɴ ᴋᴇʟɪᴋ .ᴀʟʟᴍᴇɴᴜ ᴜɴᴛᴜᴋ ᴍᴇɴᴀᴍᴘɪʟᴋᴀɴ ᴍᴇɴᴜ\n© BY HAPIS OFFICIAL', imageMessage: imageMsg,
               buttons: buttons,
               headerType: 4
}

               prep = await Hapis.prepareMessageFromContent(from,{buttonsMessage},{quoted: troli})
               Hapis.relayWAMessage(prep)
               randommusic = fs.readFileSync(`./media/audio/${randomaudio}.mp3`)
               Hapis.sendMessage(from, randommusic, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                break
        case 'allmenu':
        if (!isRegist) return freply(mess.regist)
        if (isBanned) return freply(mess.banned)
...
> 532 ${prefix}nsfw_avatar`
               ...
               buttonsMessage = {
               contentText: `${allmenu}`,
               footerText: '© BY HAPIS OFFICIAL', imageMessage: imageMsg,
               buttons: buttons,
               headerType: 4
}
...
if (isBanned) return freply(mess.banned)
freply(`━━━━━ *INFO BOT* ━━━━━
❏ BASE ORI : Hapis Official
❏ TYPE SC : nodejs
❏ DI BUAT PADA : sel, 29, mart, 2022
❏ GITHUB CREATOR : https://github.com/HapisOfficial
❏ IG CREATOR : https://instagram.com/hfzdzakwan_
*━━━━━━━━━━━━━━━━━━━━*`)
randommusic = fs.readFileSync(`./media/audio/${randomaudio}.mp3`)
Hapis.sendMessage(from, randommusic, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
...
if (isBanned) return freply(mess.banned)
freply(`━━━━━ *SCRIPT* ━━━━━
❏ LINK VIA GITHUB : https://github.com/Hapisofficial/
❏ BASE ORI : Hapis Official
❏ TYPE SC : nodejs
❏ DI BUAT PADA : sel, 29, mart, 2022
*━━━━━━━━━━━━━━━━━━━━*`)
...
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, allmenuu(prefix), MessageType.text, {quoted: troli})
break
      case 'qris':
      if (!isRegist) return freply(mess.regist)
      if (isBanned) return freply(mess.banned)
      qris =`*SCAN QRIS NYA OM, TEEF SEIHKLASNYA🗿*`
               ...
               imageMsg = (await Hapis.prepareMessageMedia(fs.readFileSync(`./media/foto/qris.jpg`), 'imageMessage', {thumbnail: fs.readFileSync(`./media/foto/qris.jpg`)})).imageMessage
...
Hapis.sendMessage(from, randomaudi, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true}) 
break
//FITUR NYAAH BANGGG
...
randommusic = fs.readFileSync(`./media/audio/${randomaudio}.mp3`)
Hapis.sendMessage(from, randommusic, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
Hapis.sendMessage(from, beliscript(prefix, nomorowner), text)
break
//JASA
//JASA RUN HEROKU
case 'jasarunheroku':
...
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
freply(`*SILAHLAN HUBUNGI*\nhttp://wa.me/6281802558378?text=bang+mau+order+jasa+run+heroku:v`)
break
//JASA UP HEROKU
case 'jasaupgithub':
case 'jasaup':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
freply(`*SILAHLAN HUBUNGI*\nhttp://wa.me/6281802558378?text=bang+mau+order+jasa+up+github:v`)
break
//STORE MENU//
//TOP UP GAME
case 'topupgame2':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, topupgame(prefix), MessageType.text, {quoted: troli})
break
case 'topupfreefire':
case 'topupff':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, topupfreefire(prefix, nomorowner), MessageType.text, {quoted: troli})
break
case 'topupmobilelegends':
case 'topupml':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, topupmobilelegends(prefix, nomorowner), MessageType.text, {quoted: troli})
break
case 'topuppubg':
case 'topuppapji':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, topuppubg(prefix, nomorowner), MessageType.text, {quoted: troli})
break
case 'topuppointblank':
case 'topuppb':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, topuppointblank(prefix, nomorowner), MessageType.text, {quoted: troli})
break
//TOP UP SALDO
case 'topupsaldo':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, topupsaldo(prefix), text)
break
case 'topupgopay':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, topupgopay(prefix, nomorowner), MessageType.text, {quoted: troli})
break
case 'topupdana':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, topupdana(prefix, nomorowner), MessageType.text, {quoted: troli})
break
case 'topupovo':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, topupovo(prefix, nomorowner), MessageType.text, {quoted: troli})
break
case 'topuppulsa':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, topuppulsa(prefix, nomorowner), MessageType.text, {quoted: troli})
break
...
// SOSMET          
      ...
      if (!isRegist) return freply(mess.regist)
      if (isBanned) return freply(mess.banned)
             freply(`Follow Instagram Owner Ya : https://www.instagram.com/muhammadhapis196453`)
             break
//casee nya
case 'grupbot':
case 'groupbot':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, groupbot(prefix), MessageType.text, {quoted: troli})
break
break
case 'infobot':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, infobot(prefix), MessageType.text, {quoted: troli})
break
case 'rules':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, rulesbot(prefix), MessageType.text, {quoted: troli})
break
case 'sewabot':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, sewabot(prefix), MessageType.text, {quoted: troli})
break
case 'buyprem':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, beliprem(prefix), MessageType.text, {quoted: troli})
break
case 'ownermenu':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, ownermenu(prefix), MessageType.text, {quoted: troli})
break
case 'downloadmenu':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, downloadmenu(prefix), MessageType.text, {quoted: troli})
break
case 'gamemenu':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, gamemenu(prefix), MessageType.text, {quoted: troli})
break      
case 'owner':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, owner(prefix), MessageType.text, {quoted: troli})
break
case 'infomenu':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, infomenu(prefix), MessageType.text, {quoted: troli})
break
case 'stickermenu':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, stickermenu(prefix), MessageType.text, {quoted: troli})
break
case 'islammenu':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, islammenu(prefix), MessageType.text, {quoted: troli})
break
case 'soundmenu':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, soundmenu(prefix), MessageType.text, {quoted: troli})
break
case 'vnmenu':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, vnmenu(prefix), MessageType.text, {quoted: troli})
break
case 'sertimenu':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, sertimenu(prefix), MessageType.text, {quoted: troli})
break
case 'ceritamenu':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, ceritamenu(prefix), MessageType.text, {quoted: troli})
break  
case 'makermenu':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, makermenu(prefix), MessageType.text, {quoted: troli})
break 
case 'toolsmenu':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from,toolsmenu(prefix), MessageType.text, {quoted: troli})
break                   
case 'dewasamenu':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, dewasamenu(prefix), MessageType.text, {quoted: troli})
break                                    
case 'othermenu':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, othermenu(prefix), MessageType.text, {quoted: troli})
break
case 'groupmenu': 
case 'grupmenu':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, groupmenu(prefix), MessageType.text, {quoted: troli})
break
case 'pricegtivd': 
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
...
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, warmenu(prefix), MessageType.text, {quoted: troli})
break
case 'funmenu':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, funmenu(prefix), MessageType.text, {quoted: troli})
break
case 'statusmenu':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, statusmenu(prefix), MessageType.text, {quoted: troli})
break
case 'beritamenu':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, beritamenu(prefix), MessageType.text, {quoted: troli})
break
case 'randommenu':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, randommenu(prefix), MessageType.text, {quoted: troli})
break
case 'hiburanmenu':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, hiburanmenu(prefix), MessageType.text, {quoted: troli})
break
case 'hewanmenu':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, hewanmenu(prefix), MessageType.text, {quoted: troli})
break
case 'searchmenu':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, searchmenu(prefix), MessageType.text, {quoted: troli})
break
case 'mememenu':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, mememenu(prefix), MessageType.text, {quoted: troli})
break
case 'asupanmenu':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, asupanmenu(prefix), MessageType.text, {quoted: troli})
break
case 'animemenu':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, animemenu(prefix), MessageType.text, {quoted: troli})
break
case 'hentaimenu':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
Hapis.sendMessage(from, hentaimenu(prefix), MessageType.text, {quoted: troli})
break                                                                
//------------------< asupan menu >------------------  
case 'asupan1':
              if (!isRegist) return freply(mess.regist)
              ...
              if (isBanned) return freply(mess.banned)
			Hapis.updatePresence(from, Presence.composing) 
				freply(mess.wait)
				...
				Hapis.sendMessage(from, asupan, video, {quoted: mek, caption: '```ASUPAN NIH:V```'})
				break        
case 'asupan2':
              if (!isRegist) return freply(mess.regist)
              if (isDewasa) return freply(mess.dewasa)
			...
			Hapis.updatePresence(from, Presence.composing) 
              ...
              videoMsg = (await Hapis.prepareMessageMedia(ukhty, "videoMessage", { thumbnail: ukhty, })).videoMessage
              ...
              videoMsg = (await Hapis.prepareMessageMedia(santuy, "videoMessage", { thumbnail: santuy, })).videoMessage
              buttonsMessage = {footerText:`${fakereply}`, videoMessage: videoMsg,
              contentText:`DONE`,buttons,headerType:5}
              prep = await Hapis.prepareMessageFromContent(from,{buttonsMessage},{quoted: troli})
              Hapis.relayWAMessage(prep)
              ...
              videoMsg = (await Hapis.prepareMessageMedia(bocil, "videoMessage", { thumbnail: bocil, })).videoMessage
              buttonsMessage = {footerText:`${fakereply}`, videoMessage: videoMsg,
              contentText:`DONE`,buttons,headerType:5}
              prep = await Hapis.prepareMessageFromContent(from,{buttonsMessage},{quoted: troli})
              Hapis.relayWAMessage(prep)
              ...
              videoMsg = (await Hapis.prepareMessageMedia(hijaber, "videoMessage", { thumbnail: hijaber, })).videoMessage
              buttonsMessage = {footerText:`${fakereply}`, videoMessage: videoMsg,
              contentText:`DONE`,buttons,headerType:5}
              prep = await Hapis.prepareMessageFromContent(from,{buttonsMessage},{quoted: troli})
              Hapis.relayWAMessage(prep)
              ...
              videoMsg = (await Hapis.prepareMessageMedia(geayubi, "videoMessage", { thumbnail: geayubi, })).videoMessage
              buttonsMessage = {footerText:`${fakereply}`, videoMessage: videoMsg,
              contentText:`DONE`,buttons,headerType:5}
              prep = await Hapis.prepareMessageFromContent(from,{buttonsMessage},{quoted: troli})
              Hapis.relayWAMessage(prep)
              ...
              videoMsg = (await Hapis.prepareMessageMedia(naura, "videoMessage", { thumbnail: naura, })).videoMessage
              buttonsMessage = {footerText:`${fakereply}`, videoMessage: videoMsg,
              contentText:`DONE`,buttons,headerType:5}
              prep = await Hapis.prepareMessageFromContent(from,{buttonsMessage},{quoted: troli})
              Hapis.relayWAMessage(prep)
              ...
              videoMsg = (await Hapis.prepareMessageMedia(bunga, "videoMessage", { thumbnail: bunga, })).videoMessage
              buttonsMessage = {footerText:`${fakereply}`, videoMessage: videoMsg,
              contentText:`DONE`,buttons,headerType:5}
              prep = await Hapis.prepareMessageFromContent(from,{buttonsMessage},{quoted: troli})
              Hapis.relayWAMessage(prep)
              ...
              videoMsg = (await Hapis.prepareMessageMedia(ayu, "videoMessage", { thumbnail: ayu, })).videoMessage
              buttonsMessage = {footerText:`${fakereply}`, videoMessage: videoMsg,
              contentText:`DONE`,buttons,headerType:5}
              prep = await Hapis.prepareMessageFromContent(from,{buttonsMessage},{quoted: troli})
              Hapis.relayWAMessage(prep)
              ...
              buttons = [{buttonId: `${prefix}caca`,buttonText:{displayText: `➡️ NEXT`},type:1}]
              videoMsg = (await Hapis.prepareMessageMedia(caca, "videoMessage", { thumbnail: caca, })).videoMessage
              buttonsMessage = {footerText:`${fakereply}`, videoMessage: videoMsg,
              contentText:`DONE`,buttons,headerType:5}
              prep = await Hapis.prepareMessageFromContent(from,{buttonsMessage},{quoted: troli})
              Hapis.relayWAMessage(prep)
              ...
              videoMsg = (await Hapis.prepareMessageMedia(delvira, "videoMessage", { thumbnail: delvira, })).videoMessage
              buttonsMessage = {footerText:`${fakereply}`, videoMessage: videoMsg,
              contentText:`DONE`,buttons,headerType:5}
              prep = await Hapis.prepareMessageFromContent(from,{buttonsMessage},{quoted: troli})
              Hapis.relayWAMessage(prep)
              ...
              videoMsg = (await Hapis.prepareMessageMedia(delvira, "videoMessage", { thumbnail: alita, })).videoMessage
              buttonsMessage = {footerText:`${fakereply}`, videoMessage: videoMsg,
              contentText:`DONE`,buttons,headerType:5}
              prep = await Hapis.prepareMessageFromContent(from,{buttonsMessage},{quoted: troli})
              Hapis.relayWAMessage(prep)
...
[ ${args[0]} ] Ketik Y/N untuk menerima atau menolak permainan`
               ...
               Hapis.sendMessage(from, starGame, text, {quoted: mek, contextInfo: { mentionedJid: [sender, args[0].replace("@", "") + "@s.whatsapp.net"],}})
...
Ket : Ketik /resetgame , Untuk Mereset Permainan Yg Ada Di Grup!`, text, {contextInfo: {mentionedJid: [player2]}})
       ...
       case 'tebakanime':
              ...
              Hapis.sendMessage(from, ini_buffer, image, { quoted: mek, caption: '*+* ```Tebak Anime```\n\n• *Petunjuk* :'+kisi_kisi+'\n• *Waktu* : 30s' }).then(() => {
       ...
       case 'tebaklagu':
              ...
              freply('*+* ```Tebak Lagu```\n\n• *Petunjuk* :'+kisi_kisi+'\n• *Waktu* : 30s')
              Hapis.sendMessage(from, ini_buffer, audio, {quoted: mek}).then(() => {
       ...
       case 'tebaktebakan':
              ...
              Hapis.sendMessage(from, '*+* ```Tebak Tebakan```\n\n• *soal* :'+pertanyaan+'\n• *kisi²* :'+kisi_kisi, text, { quoted: mek}).then(() => {
       ...
       case 'kuismath':
              ...
              jawaban = get_result.jawaban
              ini_buffer = await getBuffer(ini_image)
              Hapis.sendMessage(from, ini_buffer, image, { quoted: mek, caption: '*+* ```Kuis Matematika```\n\nSilahkan jawab soal berikut ini\n\n• *Waktu* : 50s' }).then(() => {
       ...
       case 'tebakgambar':
              ...
              kisi_kisi = jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')
              ini_buffer = await getBuffer(ini_image)
              Hapis.sendMessage(from, ini_buffer, image, { quoted: mek, caption: 'Silahkan jawab soal berikut ini\n\nPetunjuk :'+kisi_kisi+'\nWaktu : 30s' }).then(() => {
       ...
       case 'siapaaku':
              ...
              Hapis.sendMessage(from, '*+* ```Tebak Siapakah Aku```\n\n• *soal* :'+pertanyaan+'\n• *kisi²* :'+kisi_kisi, text, { quoted: mek}).then(() => {
       ...
       case 'tebakkata':
              ...
              Hapis.sendMessage(from, '*+* ```Tebak Kata```\n\n• *Soal* :'+pertanyaan+'\n• *Waktu :* 30s', text, { quoted: mek}).then(() => {
       ...
       case 'tebaklirik':
              ...
              kisi_kisi = jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')
              pertanyaan = get_result.question
              Hapis.sendMessage(from, '*+* ```Tebak Lirik```\n\n• *Soal* :'+pertanyaan+'\n• *Kisi²* :'+kisi_kisi, text, { quoted: mek}).then(() => {
      ...
      case 'tebakjenaka':
              ...
              kisi_kisi = jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')
              pertanyaan = get_result.question
              Hapis.sendMessage(from, '*+* ```Tebak Jenaka```\n\n• *Soal* :'+pertanyaan+'\n• *Kisi²* :'+kisi_kisi, text, { quoted: mek}).then(() => {
       ...
       case 'kimiakuis':
              ...
              Hapis.sendMessage(from, '*+* ```Tebak Kimia```\n\n• *Soal* :'+pertanyaan+'\n• *Waktu :* 30s', text, { quoted: mek}).then(() => {
       ...
       case 'tebakbendera':
              ...
              Hapis.sendMessage(from, '*+* ```Tebak Bendera```\n\n• *Bendera* :'+pertanyaan+'\n• *kisi²* :'+kisi_kisi, text, { quoted: mek}).then(() => {
       ...
       case 'susunkata':
              ...
              jawaban = get_result.jawaban
              pertanyaan = get_result.pertanyaan
              Hapis.sendMessage(from, '*+* ```Susun Kata```\n\n• *Soal* :'+pertanyaan+'\n• *Waktu :* 30s', text, { quoted: mek}).then(() => {
       ...
       case 'asahotak':
              ...
              kisi_kisi = jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')
              pertanyaan = get_result.pertanyaan
              Hapis.sendMessage(from, '*+* ```Asah Otak```\n\n• *soal* :'+pertanyaan+'\n• *kisi²* :'+kisi_kisi, text, { quoted: mek}).then(() => {
       ...
       case 'caklontong':
              ...
              kisi_kisi = jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')
              pertanyaan = get_result.question
              Hapis.sendMessage(from, '*+* ```Caklontong```\n\n• *soal* :'+pertanyaan+'\n• *kisi²* :'+kisi_kisi, text, { quoted: mek}).then(() => {
...
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
gopeynya = 'https://i.ibb.co/kynNYTh/Hapis2.jpg'
teksnya = `*[ PAYMENT ]*
*Gopay :* 089512545999
*Dana :* 089512545999
*Ovo :* 089512545999
*[ © BY HAPIS OFFICIAL ]*`
        Hapis.sendMessage(from, await getBuffer(gopeynya), image, {quoted: ftoko, caption: teksnya })
              break
//--------------------<const menu>--------------------
             case 'alllmenu2':
             Hapis.sendMessage(from, allmenu(prefix), MessageType.text, {quoted: troli})
             break
//------------------< Sticker Cmd >-------------------
       case 'addcmd': 
       case 'setcmd':
...
_Silahkan tunggu file media sedang dikirim mungkin butuh beberapa menit_`
        ...
        case 'google':
              ...
              vars +=  `\n═════════════════\n\n*Judul:* ${results[i].title}\n\n*Deskripsi:* ${results[i].snippet}\n\n*Link:* ${results[i].link}\n\n`
               ...
               Hapis.sendMessage(from, ss, image, {caption: vars, quoted : mek})
               }).catch(e => {
               console.log(e)
               freply(`${e}`)
})
...
case 'tiktok':
              ...
              imageMsg = (await Hapis.prepareMessageMedia(fs.readFileSync(`./media/foto/thumb.jpg`), 'imageMessage', {thumbnail: fs.readFileSync(`./media/foto/thumb.jpg`)})).imageMessage
              buttonsMessage = {footerText:'Jangan Lupa Subscribe Yt Hapis Official\n Help 1k subscriber', imageMessage: imageMsg,
              contentText:`Silahkan pilihan media yg mau di download kak:v`,buttons,headerType:4}
              prep = await Hapis.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek})
              Hapis.relayWAMessage(prep)
              break
      case 'ttaudio': 
      case 'tiktokmusic': 
      case 'tiktokaudio':
             ...
             Hapis.sendMessage(from, get_audio, audio, { mimetype: Mimetype.mp4Audio, quoted: mek })
             break
      case 'fb':
      case 'facebook':
              if (!isRegist) return freply(mess.regist)
...
Hapis.sendMessage(from, buffungu, image, {quoted: troli, caption : teks})
break
            case 'ssweb2':
            case 'ssurl2':
            case 'sslink2':
            ...
            Hapis.sendMessage(from, anu, image, {caption: `BERHASIL KAK\n\nPlease Support *Aprilia*`, quoted: mek})
            break
	        case 'encimg':
		    case 'encfoto':
            if (!isRegist) return freply(mess.regist)
			...
			const media = await Hapis.downloadAndSaveMediaMessage(encmedia)
			freply(mess.wait)
...
case 'art':
                ...
                Hapis.sendMessage(from, gambar, image, { quoted: mek })
                })
                break
                case 'hentai':
                case 'randomhentai':
...
//HENTAI MENU
       case 'nhentaipdf':
             ...
             Hapis.sendMessage(from, ini_image, image, { caption: teks, quoted: mek })
             anu = await fetchJson(`https://api.lolhuman.xyz/api/nhentaipdf/${henid}?apikey=${lolhumanapi}`)
             pdf = await getBuffer(anu.result)
             Hapis.sendMessage(from, pdf, document, { quoted: mek, mimetype: Mimetype.pdf, filename: `${get_result.title_romaji}.pdf`, thumbnail: ini_image })
             } catch (e) {
             console.log(e)
             freply(String(e))
}
       ...
       case 'manga':
              ...
              await Hapis.sendMessage(from, thumbnail, image, { quoted: mek, caption: ini_txt })
              break
       case 'doujindesu':
              if (!isRegist) return freply(mess.regist)
              if (isBanned) return freply(mess.banned)
             ...
             Hapis.sendMessage(from, buffer, image, {caption: xixixai, quoted: mek})
             } catch (e) {
             console.log(e)
             freply(String(e))
}
      ...
      case 'kusonime':
             ...
             await Hapis.sendMessage(from, ini_buffer, image, { quoted: mek, caption: ini_txt })
             break
       case 'otakudesu':
              if (!isRegist) return freply(mess.regist)
              if (isBanned) return freply(mess.banned)
       ...
       if (isBanned) return freply(mess.banned)
              reply(mess.wait)
              getBuffer(`https://api.lolhuman.xyz/api/random/${command}?apikey=${lolhumanapi}`).then((gambar) => {
              Hapis.sendMessage(from, gambar, image, { quoted: mek, thumbnail: Buffer.alloc(0) })
})
              break
       ...
       case 'storyanime':
              ...
              Hapis.sendMessage(from, buffer, video, { quoted: mek })
              break
       case 'nekopoi3d':
       case '3dnekopoi':
       case '3dnekopoilast':
        ...
        case 'otakuongoing':              
              ...
              if (isBanned) return freply(mess.banned)
               ...
               Hapis.sendMessage(from, buff, image, {quoted: mek, caption: ot})
               break
//ANIME MENU
            case 'waifu':
              if (!isRegist) return freply(mess.regist)
...
buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1},{buttonId:`${prefix}nhentaibot`,buttonText:{displayText:'NHENTAI BOT'},type:1}]
              imageMsg = ( await Hapis.prepareMessage(from, inifile, 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
...
break
       ...
       case 'wallml':              
		      ...
		      buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1},{buttonId:`${prefix}nhentaibot`,buttonText:{displayText:'NHENTAI BOT'},type:1}]
              imageMsg = ( await Hapis.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
              buttonsMessage = {footerText:'Jangan Lupa Donasi Ya Kak ☕', imageMessage: imageMsg,
              contentText:`klik Next untuk ke gambar selanjut nya`,buttons,headerType:4}
              prep = await Hapis.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek})
              Hapis.relayWAMessage(prep)
...
Hapis.sendMessage (from, buf, image, {quoted: mek, caption: sayang})
sound = await getBuffer (anu.url)
Hapis.sendMessage(from, sound, audio, {mimetype: 'audio/mp4', quoted: mek})
break
case 'play3':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
...
Hapis.sendMessage(from, mp4, video)
break
case 'mp3':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
...
Hapis.sendMessage(from, mp3, audio)
break
          case 'lirik':
              if (!isRegist) return freply(mess.regist)
              if (isBanned) return freply(mess.banned)
...
`
        ...
        case 'pinterest':
              ...
              if (args.length == 0) return freply(`Example: ${prefix + command} loli kawaii`)
                    ...
                    await Hapis.sendMessage(from, ini_buffer, image, { quoted: mek })
                    break      
        case 'shopee':
              if (!isRegist) return freply(mess.regist)
              if (isBanned) return freply(mess.banned)
...
\`\`\`• Link : ${get_data[i].url}\`\`\``
              ...
              Hapis.sendMessage(from, ini_buffer, image, { quoted: mek, caption: teks })
              } catch {
...
\`\`\`• Link : ${i.url}\`\`\``
       ...
       case 'tourl':
              ...
              if (isBanned) return freply(mess.banned)
               ...
               owgi = await Hapis.downloadMediaMessage(boij)
       ...
       case 'img2url':
              ...
              if (isBanned) return freply(mess.banned)
               ...
               var media = await  Hapis.downloadAndSaveMediaMessage(encmedia)       
               imgbb('39d895963468b814fad0514bd28787e2', media)
              .then(data => {
               var caps = `*_IMAGE TO URL_*\n\n*~>  ID :* ${data.id}\n*~>  MimeType :* ${data.image.mime}\n*~>  Extension :* ${data.image.extension}\n*~>  URL :* ${data.display_url}`
               ibb = fs.readFileSync(media)
               Hapis.sendMessage(from, ibb, image, { quoted: mek, caption: caps})
})
              .catch(err => {
               throw err
})
         ...
         case 'asupan':
              ...
              .then(async (body) => {
               ...
               sendMediaURL(from, `http://sansekai.my.id/ptl_repost/${asupanx}`, '\`\`\`ASUPAN BRO GUA DAPAT DARI HAPIS OFFICIAL\`\`\`')
        ...
        case 'tulis':
               ...
               Hapis.sendMessage(from, buff, image, {quoted: mek, caption: mess.success}).catch(e => {
...
//------------------< Level >-------------------
      case 'level': 
              ...
              try {
              profilePic = await Hapis.getProfilePicture(sender)
              } catch {
              profilePic = errorImg
}
              ...
              Hapis.sendMessage(from, buffer, image, { caption: teks, quoted: mek})
              break
...
//------------------< Stalk >-------------------
      ...
      case 'githubstalk':
              if (!isRegist) return freply(mess.regist)
              if (isBanned) return freply(mess.banned)
              if (args.length == 0) return freply(`Example: ${prefix + command} Hapis-chan02`)
              freply(mess.wait)
              username = args[0]
...
\`\`\`• Link : ${ini_result.url}\`\`\`
`
             Hapis.sendMessage(from, ini_buffer, image, { caption: ini_txt, thumbnail: Buffer.alloc(0) })
             break
      case 'stalkig':
      case 'igstalk':
              if (!isRegist) return freply(mess.regist)
              if (isBanned) return freply(mess.banned)
             if (args.length == 0) return freply(`Example: ${prefix + command} Hapis.chan26`)
             freply(mess.wait)
             username = args[0]
...
`
              Hapis.sendMessage(from, pp_tt, image, { quoted: mek, caption: ini_txt, thumbnail: Buffer.alloc(0) })
              break
       case 'iguser':
              if (!isRegist) return freply(mess.regist)
              if (isBanned) return freply(mess.banned)
...
//------------------< media/sticker/Tools >-------------------
      ...
      case 'doge':
       ...
       case 'telestiker':
              ...
              Hapis.sendMessage(from, ini_buffer, sticker, {})
}
              break
       case 'semoji':
       case 'emoji':
              ...
              await Hapis.sendMessage(from, ini_buffer, sticker, { quoted: mek })
              break
case 'ttp2':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
...
case 'ttp':
              if (!isRegist) return freply(mess.regist)
              if (isBanned) return freply(mess.banned)
              if (args.length == 0) return freply(`Example: ${prefix + command} Hapis Official`)
              woy = args.join(" ")
              freply(mess.wait)
              buffer = await getBuffer(`http://zekais-api.herokuapp.com/text2png?text=${woy}&color=white`)
              Hapis.sendMessage(from, buffer, sticker, { quoted: mek })
              break
       case 'attp':
              if (!isRegist) return freply(mess.regist)
              if (isBanned) return freply(mess.banned)
       ...
       case 'gifstiker':
            ...
            const media = await Hapis.downloadAndSaveMediaMessage(encmedia)
                ...
                .on('end', function () {
                console.log('Finish')
                Hapis.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
                 fs.unlinkSync(media)
                fs.unlinkSync(ran)
                })
       ...
       case 'colong':
              ...
              media = await Hapis.downloadAndSaveMediaMessage(encmedia)
              anu = args.join(' ').split('|')
              satu = anu[0] !== '' ? anu[0] : `${pushname}`
              dua = typeof anu[1] !== 'undefined' ? anu[1] : `UwU`
              require('./lib/fetch.js').createExif(satu, dua)
              require('./lib/fetch.js').modStick(media, Hapis, mek, from)
              break
       case 'delwm':
              if (!isRegist) return freply(mess.regist)
              if (isBanned) return freply(mess.banned)
       ...
       case 'swm':
             ...
             .on('error', (e) => {
              console.log(e)
              Hapis.sendMessage(from, 'Terjadi kesalahan', 'conversation', { quoted: mek })
              fs.unlinkSync(media)
})
             .on('end', () => {
            _out = getRandom('.webp')
             ...
             .on('exit', () => {
              Hapis.sendMessage(from, fs.readFileSync(_out),'stickerMessage', { quoted: mek })
              fs.unlinkSync(out)
              fs.unlinkSync(_out)
              fs.unlinkSync(media)
})
      ...
      case 'toimg':
              ...
              Hapis.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih'})
              fs.unlinkSync(ran)
})
              break
              case 'smeme': 
...
owgi = await  Hapis.downloadAndSaveMediaMessage(ger)
       ...
       case 'memegen':                    
        ...
        case 'togif':
              ...
              if (isBanned) return freply(mess.banned)
               ...
               mediaaa = await Hapis.downloadAndSaveMediaMessage(encmediaaa)
               a = await webp2gifFile(mediaaa)
               mp4 = await getBuffer(a.result)
               Hapis.sendMessage(from, mp4, video, {mimetype: 'video/gif', quoted: mek, caption: mess.success})
        ...
        case 'tovideo':
              ...
              if (isBanned) return freply(mess.banned)
               ...
               a = await webp2gifFile(mediaaa)
               mp4 = await getBuffer(a.result)
               Hapis.sendMessage(from, mp4, video, {mimetype: 'video/mp4', quoted: mek, caption: mess.success})
               fs.unlinkSync(mediaaa)
               } else {
               freply(mess.wrongFormat)
}
        ...
        case 'tomp3':
              ...
              if (isBanned) return freply(mess.banned)
               ...
               Hapis.sendMessage(from, buffer453, audio, { mimetype: 'audio/mp4', quoted: mek })
               fs.unlinkSync(ran)
})
               } else {
               freply(mess.wrongFormat)
...
//------------------<18+ Menu>-----------------------   
       case 'randombokep':
              ...
              Hapis.sendMessage(from, '*PERMINTAAN:* '+bokep+'\n*DOSA TANGGUNG PRIBADI*\n*NI BRO FREE BUAT KAMU DOWNLOAD SENDIRI:* '+ kep, text, { quoted: ftoko, caption: `NI BOKEP SAYA DAPAT DARI *© BY HAPIS OFFICIAL* DOSA TANGGUNG SENDIRI🗿`})
              break
                case 'xnxx':
              if (!isRegist) return freply(mess.regist)
              if (isDewasa) return freply(mess.dewasa)
              ...
              if (!isPremium) return freply(mess.premium)
                    ...
                    Hapis.sendMessage(from, thumbnail, image, { quoted: mek, caption: ini_txt })
                    break
case 'xnxxsearch':
              if (!isRegist) return freply(mess.regist)
              if (isDewasa) return freply(mess.dewasa)
...
break 
                    ...
                    negara = args.join(' ')
                    texxt = args.join(' ')
					Hapis.sendMessage(from, negara(), texxt)
					break
                    case 'tts':
                    if (!isRegist) return freply(mess.regist)
                    if (isBanned) return freply(mess.banned)
				    if (args.length < 1) return Hapis.sendMessage(from, 'Diperlukan kode bahasa kak!!', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return Hapis.sendMessage(from, 'Mana teks yang mau di jadiin suara? suara setan kah?', text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 500
					...
					: gtts.save(ranm, dtt, function() {
						exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
							...
							Hapis.sendMessage(from, buffer, audio, {quoted: mek, ptt:true})
							fs.unlinkSync(rano)
						})
					})
					break
...
Hapis.sendMessage(from, tapib1, document, { quoted: mek, filename:`Hapis Official ~ 404 ${danz}.mp3`, mimetype: 'audio/application' })
...
buf = Mfake
imeu = await Hapis.prepareMessage('0@s.whatsapp.net', buf, image) 
imeg = imeu.message.imageMessage
res = await Hapis.prepareMessageFromContent(from,{
"groupInviteMessage": {
"groupJid": "6281802558378-1328272333@g.us",
"inviteCode": "wFHwtOxGQN8OwK2x",
"inviteExpiration": "162533333338378",
"groupName": `Hapis Official ~ 404 ${danz}`,
...
Hapis.toggleDisappearingMessages(from, 'Awoakwoakwoak')
Hapis.relayWAMessage(res)
break
case 'plp':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
...
"businessOwnerJid": "6281802558378@s.whatsapp.net",
"contextInfo": {
"forwardingScore": 3,
"isForwarded": true
}
...
break
      ...
      case 'botstat': 
              if (!isRegist) return freply(mess.regist)
              if (isBanned) return freply(mess.banned)
              groups = Hapis.chats.array.filter(v => v.jid.endsWith('g.us'))
              privat = Hapis.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
              ram2 = `${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / ${Math.round(require('os').totalmem / 1024 / 1024)}MB`
              charger = `${charging ? 'lagi dicas' : 'ga dicas'}`
              uptime = process.uptime();
              timestampe = speed();
              totalChat = await Hapis.chats.all()
              latensie = speed() - timestampe
...
\`\`\`• Wa Version: ${Hapis.user.phone.wa_version}\`\`\`
\`\`\`• Os Version: ${Hapis.user.phone.os_version}\`\`\`
\`\`\`• Device Manufacturer: ${Hapis.user.phone.device_manufacturer}\`\`\`
\`\`\`• Device Model: ${Hapis.user.phone.device_model}\`\`\`
\`\`\`• Os Build Number: ${Hapis.user.phone.os_build_number}\`\`\``
             freply(teks)
             break  
//------------------< Owner Menu >-------------------
      case 'bc':
      case 'broadcast': 
             ...
             anu = await Hapis.chats.all()
             if (isMedia && !mek.message.videoMessage || isQuotedImage) {
             const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
             bc = await Hapis.downloadMediaMessage(encmedia)
             for (let _ of anu) {
             Hapis.sendMessage(_.jid, bc, image, {quoted:freply,caption: `*「 PESAN SIARAN BOT 」*\n\n${body.slice(4)}`})
}
             freply('Suksess broadcast')
             } else {
             for (let _ of anu) {
Hapis.sendMessage(_.jid, 
			{"contentText": `*「 Siaran ${namabot} 」*\n.Ijin Siaran\n*Isi Pesan :* ${body.slice(4)}`,
			"footerText": '© BY HAPIS OFFICIAL',
			"buttons": [
			...
			"buttonText": {"displayText": "MENU"
             ...
             break
				 ...
				 Hapis.chatRead (from)
					...
					Hapis.blockUser (`${body.slice(7)}@c.us`, "add")
					Hapis.sendMessage(from, `perintah Diterima, memblokir ${body.slice(7)}@c.us`, text)
					break
                    case 'unblock': 
                    if (!isRegist) return freply(mess.regist)
                    if (isBanned) return freply(mess.banned)
					if (!isGroup) return freply(mess.only.group)
					if (!isOwner) return freply(mess.only.ownerB)
				    Hapis.blockUser (`${body.slice(9)}@c.us`, "remove")
					Hapis.sendMessage(from, `Perintah Diterima, membuka ${body.slice(9)}@c.us`, text)
					break   				
					case 'setppbot': 
                    if (!isRegist) return freply(mess.regist)
                    if (isBanned) return freply(mess.banned)
					...
					media = await Hapis.downloadAndSaveMediaMessage(enmedia)
					await Hapis.updateProfilePicture(botNumber, media)
...
break 
        ...
        if (isBanned) return freply(mess.banned)
        if (!isOwner) return freply(mess.only.ownerB)
					Hapis.deleteMessage(from, { id: mek.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true })
					break
      case 'addupdate': 
             if (!isRegist) return freply(mess.regist)
             if (isBanned) return freply(mess.banned)
      ...
      case 'join':  
             ...
             fak = Hapis.query({ json: ['action', 'invite', link],
             expect200: true })
...
case 'join3':
            ...
            var response = await Hapis.acceptInvite(codeInvite)
            fakeitem('SUKSES')
            } catch {
            fakeitem('LINK ERROR!')
            }
      ...
      case 'clearall': 
             ...
             if (!isOwner) return  freply(mess.only.owner)
             anu = await Hapis.chats.all()
             Hapis.setMaxListeners(25)
             for (let _ of anu) {
             Hapis.deleteChat(_.jid)
}
      ...
      case 'leaveall': 
             ...
             if (isBanned) return freply(mess.banned)
             if (!isOwner) return  freply(mess.only.owner)
             let totalgroup = Hapis.chats.array.filter(u => u.jid.endsWith('@g.us')).map(u => u.jid)
...
//-----------< vn menu >-----------\\
					...
					exec(`ffmpeg -i ${media} -filter:a "atempo=0.5,asetrate=65100" ${ran}`, (err, stderr, stdout) => {
						...
						Hapis.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, quoted: troli})
						fs.unlinkSync(ran)
					})
				break 
case 'addstik': 
	         ...
	         if (!isOwner) return freply(mess.only.owner)
				...
				delb = await Hapis.downloadMediaMessage(boij)
				setiker.push(`${svst}`)
				fs.writeFileSync(`./temp/stick/${svst}.webp`, delb)
				fs.writeFileSync('./temp/stick.json', JSON.stringify(setiker))
				Hapis.sendMessage(from, `Sukses Menambahkan Sticker\nCek dengan cara ${prefix}liststik`, MessageType.text, { quoted: troli})
				break
case 'addimg': 
                if (!isRegist) return freply(mess.regist)
                if (isBanned) return freply(mess.banned)
...
case 'liststik': 
             ...
             if (isBanned) return freply(mess.banned)
				...
				Hapis.sendMessage(from, teks.trim(), extendedText, { quoted: troli, contextInfo: { "mentionedJid": setiker } })
				break				
case 'listimg': 
             if (!isRegist) return freply(mess.regist)
             if (isBanned) return freply(mess.banned)
...
case 'listvid': 
             ...
             if (isBanned) return freply(mess.banned)
				...
				Hapis.sendMessage(from, teks.trim(), extendedText, { quoted: troli, contextInfo: { "mentionedJid": imagenye } })
				break				
case 'listvn': 
             if (!isRegist) return freply(mess.regist)
             if (isBanned) return freply(mess.banned)
				...
				Hapis.sendMessage(from, teks.trim(), extendedText, { quoted: troli, contextInfo: { "mentionedJid": audionye } })
				break	
case 'getstik': 
             if (!isRegist) return freply(mess.regist)
             if (isBanned) return freply(mess.banned)
				...
				Hapis.sendMessage(from, result, sticker,{quoted:troli})
				} catch {
...
case 'getimg': 
             ...
             if (isBanned) return freply(mess.banned)
				...
				Hapis.sendMessage(from, buffer, image, { quoted: troli, caption: `Result From Database : ${namastc}.jpeg` })
				} catch {
				  freply('Pack tidak terdaftar')
				}
				break
...
case 'getvid': 
             ...
             if (isBanned) return freply(mess.banned)
				...
				Hapis.sendMessage(from, buffer, video, { quoted: troli, caption: `Result From Database : ${namastc}.mp4` })
				} catch {
				  freply('Pack tidak terdaftar')
				}
				break				
case 'getvn': 
             ...
             if (isBanned) return freply(mess.banned)
				...
				Hapis.sendMessage(from, buffer, audio, { mimetype: 'audio/mp4', quoted: troli, ptt: true })
				} catch {
				  freply('Pack tidak terdaftar')
				}
				break
...
case 'linkgc': 
             ...
             if (isBanned) return freply(mess.banned)
				if (!isGroup) return freply(mess.only.group)
				linkgc = await Hapis.groupInviteCode (from)
				yeh = `https://chat.whatsapp.com/${linkgc}\n\nlink Group *${groupName}*`
				Hapis.sendMessage(from, yeh, text, {quoted: mek})
				break
            case 'kick': 
             if (!isRegist) return freply(mess.regist)
             if (isBanned) return freply(mess.banned)
      ...
      case 'promote': 
             ...
             Hapis.groupMakeAdmin(from, mems_ids)
             } else {
             Hapis.groupMakeAdmin(from, entah)
}
             } else {
             entah = mek.message.extendedTextMessage.contextInfo.participant
             Hapis.groupMakeAdmin(from, [entah])
      ...
      case 'demote': 
             ...
             Hapis.groupDemoteAdmin(from, mems_ids)
             } else {
             Hapis.groupDemoteAdmin(from, [entah[0]])
}
             } else {
             entah = mek.message.extendedTextMessage.contextInfo.participant
             Hapis.groupDemoteAdmin(from, [entah])
}
             break
       case 'setgrupname': 
              if (!isRegist) return freply(mess.regist)
              ...
              Hapis.groupUpdateSubject(from, q)
       ...
       case 'setdesc': 
             ...
             if (isBanned) return freply(mess.banned)
              ...
              Hapis.groupUpdateDescription(from, q)
             .then((res) => freply(jsonformat(res)))
             .catch((err) => freply(jsonformat(err)))
              break
       case 'setppgrup': 
             ...
             if (isBanned) return freply(mess.banned)
              ...
              let media = await Hapis.downloadMediaMessage(encmedia)
              Hapis.updateProfilePicture(from, media)
             .then((res) => freply(jsonformat(res)))
             .catch((err) => freply(jsonformat(err)))
              } else {
       ...
       case 'profile': 
             ...
             if (isBanned) return freply(mess.banned)
              ...
              Hapis.updatePresence(from, Presence.composing)
              try {
              profil = await Hapis.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
              } catch {
              profil = errorImg
}
              thu = await Hapis.getStatus(`${sender.split('@')[0]}@s.whatsapp.net`, MessageType.text)
              me = Hapis.user
              uptime = process.uptime()
              profile = `-----[ *USER INFO* ]-----\n\n➸ *Username:* ${pushname}\n➸ *Status:* ${thu.status}\n➸ *Premium*: ${isPremium ? 'Ya' : 'No'}\n➸ *Admin*: ${isGroupAdmins ? 'Ya' : 'No'}\n➸ *Prefix :* Multi Prefix\n\n=_=_=_=_=_=_=_=_=_=_=_=_=\n\nYour progress:\n➸ *Level*: ${Levelnye}\n➸ *XP*: ${Xpluu} / ${requiredXplu}`
              buff = await getBuffer(profil)
              Hapis.sendMessage(from, buff, image, {quoted: freply, caption: profile})
              break
       case 'afk':  
              if (!isRegist) return freply(mess.regist)
              if (isBanned) return freply(mess.banned)
       ...
       case 'groupinfo': 
             ...
             if (!isGroup) return freply(mess.only.group)
              try {
              var pic = await Hapis.getProfilePicture(from)
              } catch {
              var pic = 'https://i.ibb.co/Tq7d7TZ/age-hananta-495-photo.png'
}
              let ingfo = `*G R O U P I N F O*\n\n*Name :* ${groupName}\n*ID Grup :* ${from}\n*Dibuat :* ${moment(`${groupMetadata.creation}` * 1000).tz('Asia/Jakarta').format('DD/MM/YYYY HH:mm:ss')}\n*Owner Grup :* @${groupMetadata.owner.split('@')[0]}\n*Jumlah Admin :* ${groupAdmins.length}\n*Jumlah Peserta :* ${groupMembers.length}\n*Welcome :* ${isWelkom ? 'Aktif' : 'Mati'}\n*AntiLink :* ${isAntiLink ? 'Aktif' : 'Mati'}\n*Desc :* \n${groupMetadata.desc}`
              Hapis.sendMessage(from, await getBuffer(pic), image, {quoted: mek, caption: ingfo, contextInfo: {"mentionedJid": [groupMetadata.owner.replace('@c.us', '@s.whatsapp.net')]}})
              break
       case 'tagall': 
              if (!isRegist) return freply(mess.regist)
              if (isBanned) return freply(mess.banned)
       ...
       case 'listonline':
             ...
             let online = [...Object.keys(Hapis.chats.get(ido).presences), Hapis.user.jid]
             Hapis.sendMessage(from, 'List Online:\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join `\n`, text, { quoted: mek, contextInfo: { mentionedJid: online }})
             } catch (e) {
             freply(`${e}`)
}
             break
      ...
      case 'sider': 
             ...
             if(!isGroup) return freply(mess.only.group)
             try {
             infom = await Hapis.messageInfo(from, mek.message.extendedTextMessage.contextInfo.stanzaId)
             tagg = []
             teks = `*• Dibaca oleh:*\n\n`
...
//------------------< Fun >-------------------
       ...
       case 'toxic':
        ...
        case 'citacita':
             ...
             if (isBanned) return freply(mess.banned)
              ...
              Hapis.sendMessage(from, cita2, audio,{mimetype: 'audio/mp4', ptt:true, quoted: mek})
              break
       case 'apakah':
             if (!isRegist) return freply(mess.regist)
             if (isBanned) return freply(mess.banned)
              ...
              Hapis.sendMessage(from, '*Pertanyaan :* '+apakah+'\n*Jawaban :* '+ kah, text, { quoted: mek })
              break
       case 'rate':
       case 'nilai':
             if (!isRegist) return freply(mess.regist)
             if (isBanned) return freply(mess.banned)
              ...
              Hapis.sendMessage(from, '*Pertanyaan :* '+rate+'\n*Jawaban :* '+ te+'%', text, { quoted: mek })
              break
       case 'gantengcek':
       case 'cekganteng':
             if (!isRegist) return freply(mess.regist)
             if (isBanned) return freply(mess.banned)
              ...
              Hapis.sendMessage(from, '*Pertanyaan :* '+ganteng+'\n*Jawaban :* '+ teng+'%', text, { quoted: mek })
              break
       case 'cantikcek':
       case 'cekcantik':
             if (!isRegist) return freply(mess.regist)
             if (isBanned) return freply(mess.banned)
              ...
              Hapis.sendMessage(from, '*Pertanyaan :* '+cantik+'\n*Jawaban :* '+ tik+'%', text, { quoted: mek })
              break
       case 'cekwatak':
             if (!isRegist) return freply(mess.regist)
             if (isBanned) return freply(mess.banned)
              var namao = pushname
              var prfx = await Hapis.getProfilePicture(sender)
       ...
       case 'hobby':
             ...
             if (isBanned) return freply(mess.banned)
              ...
              Hapis.sendMessage(from, 'Pertanyaan : *'+hobby+'*\n\nJawaban : '+ by, text, { quoted: mek })
              break
       case 'bisakah':
             if (!isRegist) return freply(mess.regist)
             if (isBanned) return freply(mess.banned)
              ...
              Hapis.sendMessage(from, '*Pertanyaan :* '+bisakah+'\n*Jawaban :* '+ keh, text, { quoted: mek })
              break
       case 'kapankah':
             if (!isRegist) return freply(mess.regist)
             if (isBanned) return freply(mess.banned)
              ...
              Hapis.sendMessage(from, '*Pertanyaan :* '+kapankah+'\n*Jawaban :* '+ koh, text, { quoted: mek })
              break
       case 'truth':
             if (!isRegist) return freply(mess.regist)
             if (isBanned) return freply(mess.banned)
              ...
              Hapis.sendMessage(from, truteh, image, { caption: '*Truth*\n\n'+ ttrth, quoted: mek })
              break
       case 'dare':
             if (!isRegist) return freply(mess.regist)
             if (isBanned) return freply(mess.banned)
              ...
              Hapis.sendMessage(from, buffer, image, { quoted: mek, caption: '*Dare*\n\n'+ der })
              break		
       case 'jadian':
             if (!isRegist) return freply(mess.regist)
             if (isBanned) return freply(mess.banned)
...
//------------------< Lainnya >-------------------
      ...
      case 'caripesan':
             ...
             freply(mess.wait)
             cok = await Hapis.searchMessages(`${ve}`, from, batas,1) 
             ...
             Hapis.sendMessage(from, `Ditemukan!`, text, {sendEphemeral: true, quoted: cok.messages[i]}) 
}
}
             } catch (e) {
       ...
       case 'bugreport':
              ...
              Hapis.sendMessage('6281802558378@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
              break
       case 'readall':
              if (!isRegist) return freply(mess.regist)
              if (isBanned) return freply(mess.banned)
              totalchat.map( async ({ jid }) => {
              await Hapis.chatRead(jid)
})
...
case 'asmaulhusna':
 ...
 case 'alquranaudio':
              ...
              if (isBanned) return freply(mess.banned)
                    ...
                    Hapis.sendMessage(from, ini_buffer, audio, { quoted: mek, mimetype: Mimetype.mp4Audio })      
                    break                                         
//------------------< serti menu >-------------------   
case 'sertitolol': 				
              if (!isRegist) return freply(mess.regist)
              if (isBanned) return freply(mess.banned)
				...
				Hapis.sendMessage(from, zhain, image, { quoted: mek, caption: 'Nih Dah Jadi' })
				break
//------------------< cerita menu >-------------------
case 'cerpen':
                    if (!isRegist) return freply(mess.regist)
 		...
 		case 'group':
              ...
              if (isBanned) return freply(mess.banned)
					...
					if (args[0] === 'buka') {
						...
						Hapis.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'tutup') {
						freply(`*BERHASIL MENUTUP GROUP*`)
						Hapis.groupSettingChange(from, GroupSettingChange.messageSend, true)
					}
					break   
//--------------<soundnya vang:v>-----------------
      ...
      case 'assalamualaikum':
             ...
             Hapis.sendMessage(from, assalamualaikum, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
             break
             case 'sound1':
satu = fs.readFileSync('./media/audio/sound1.mp3');
Hapis.sendMessage(from, satu, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound2':
dua = fs.readFileSync('./media/audio/sound2.mp3');
Hapis.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound3':
tiga = fs.readFileSync('./media/audio/sound3.mp3');
Hapis.sendMessage(from, tiga, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound4':
empat = fs.readFileSync('./media/audio/sound4.mp3');
Hapis.sendMessage(from, empat, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound5':
lima = fs.readFileSync('./media/audio/sound5.mp3');
Hapis.sendMessage(from, lima, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound6':
enam = fs.readFileSync('./media/audio/sound6.mp3');
Hapis.sendMessage(from, enam, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound7':
tujuh = fs.readFileSync('./media/audio/sound7.mp3');
Hapis.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break									
case 'sound8':
delapan = fs.readFileSync('./media/audio/sound8.mp3');
Hapis.sendMessage(from, delapan, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound9':
sembilan = fs.readFileSync('./media/audio/sound9.mp3');
Hapis.sendMessage(from, sembilan, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound10':
sepuluh = fs.readFileSync('./media/audio/sound10.mp3');
Hapis.sendMessage(from, sepuluh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound11':
sebelas = fs.readFileSync('./media/audio/sound11.mp3');
Hapis.sendMessage(from, sebelas, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound12':
duabelas = fs.readFileSync('./media/audio/sound12.mp3');
Hapis.sendMessage(from, duabelas, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound13':
tigabelas = fs.readFileSync('./media/audio/sound13.mp3');
Hapis.sendMessage(from, tigabelas, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound14':
empatbelas = fs.readFileSync('./media/audio/sound14.mp3');
Hapis.sendMessage(from, empatbelas, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound15':
limabelas = fs.readFileSync('./media/audio/sound15.mp3');
Hapis.sendMessage(from, limabelas, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
...
reply = fs.readFileSync('./media/ara2.mp3');
             Hapis.sendMessage(from, reply, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
             break
...
Hapis.sendMessage(from, oke, image, {quoted: mek, caption: '*Nih Kak Dah Jadi*'})
break
case 'hartatahta': 
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
...
Hapis.sendMessage(from, harta, image, {quoted: mek, caption: '*Nih Kak Dah Jadi*'})
break
...
Hapis.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'pornhub':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
...
Hapis.sendMessage(from, girlneko, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'sadboy':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
...
• Pemenangnya adalah [ @${winR} ] `
	   Hapis.sendMessage(from, starGame, text, {quoted: troli, contextInfo: { mentionedJid: [winR + "@s.whatsapp.net", gelutSkuy.Z + "@s.whatsapp.net", gelutSkuy.Y + "@s.whatsapp.net",]}})
		fs.unlinkSync("./database/game/" + from + ".json");
	} else if (sender == `${gelutSkuy.Y}@s.whatsapp.net` &&  budy.toLowerCase() == 'n') {
		Hapis.sendMessage(from, `👑 Game Gelud Rejected 🤙🏻
...
Giliran = @${tty.player1.split('@')[0]}
  ...
  Hapis.sendMessage(from, ucapan, text, {quoted: troli, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
  }
...
naa = ky_ttt.filter(toek => !toek.id.includes(from)) 
ky_ttt = naa
Hapis.sendMessage(from, `Yahh @${tty.player2.split('@')[0]} Menolak:(`,text,{quoted:troli,contextInfo:{mentionedJid:[tty.player2]}})
}
}
...
${ttt}`
Hapis.sendMessage(from, ucapan1, text, {quoted: troli, contextInfo:{mentionedJid: [tty.player1]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
...
${ttt}`
Hapis.sendMessage(from, ucapan1, text, {quoted:troli, contextInfo:{mentionedJid: [tty.player2]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
...
if (!mek.key.fromMe && !isOwner) return
try {
return Hapis.sendMessage(from, JSON.stringify(eval(budy.slice(2)),null,'\t'),text, {quoted: mek})
} catch(err) {
e = String(err)
freply(e)
}
...
Hapis.sendMessage(from, {text:teks, jpegThumbnail:fs.readFileSync('./media/foto/emror.jpg')}, 'extendedTextMessage', {sendEphemeral:true, quoted:mek, contextInfo:{ forwardingScore:508, isForwarded:true, mentionedJid:[senderr]}})
}
	} 
